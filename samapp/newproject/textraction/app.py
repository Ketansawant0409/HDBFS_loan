...
def lambda_handler(event, context):
    ...
    input_data = event  # The input data is passed from the Step Functions payload
    
    # Extract the unique_code from the response of the first Lambda
    first_lambda_response = input_data.get("Payload","")
    unique_code_start_index = first_lambda_response.rfind("unique_id") + len("unique_id")
    unique_code = first_lambda_response[unique_code_start_index:].lstrip()
    print("space:"+unique_code)
